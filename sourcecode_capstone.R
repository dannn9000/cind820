...
# 6. Setup the train/test splits. 
      ...
      #80/20 prep train and test set (this is not refreshed and will serve as 1st measurement out of m repeated training-test split)
          
          #positive label sample 80/20 train/test set
          pos_df <- tweet_df_merge[tweet_df_merge$sentiment_label=="POS",]
          pos_train_df <- sample_frac(pos_df, .8)
          pos_test_df <- setdiff(pos_df, pos_train_df)
          
          #negative label sample 80/20 train/test set
          neg_df <- tweet_df_merge[tweet_df_merge$sentiment_label=="NEG",]
          neg_train_df <- sample_frac(neg_df, .8)
          neg_test_df <- setdiff(neg_df, neg_train_df)
          
          #added train/test label in column name called set
          train_df <- union(pos_train_df, neg_train_df)
          train_df$set <- 'train'
          test_df <- union(pos_test_df, neg_test_df)
          test_df$set <- 'test'
          
          #form train and test set, and retain previous row index and its sorting
          tweet_df_8020 <- union(train_df, test_df)
          ...
          #provide summary of training and test sets
...
# 8. Train random forest
      ...
      #Random Forest for 80/20, n=3, results
          set.seed(71)
          rf_8020_n3 <- randomForest(target~.,data=TFIDF_df_train_8020_n3) 
          print(rf_8020_n30)
          
          #results for training set
          p1_8020_n3<- predict(rf_8020_n3, TFIDF_df_train_8020_n3) 
          confusionMatrix(p1_8020_n3, TFIDF_df_train_8020_n3$target)
          
          #results for test set
          p2_8020_n3<- predict(rf_8020_n3, TFIDF_df_test_8020_n3) 
          confusionMatrix(p2_8020_n3, TFIDF_df_test_8020_n3$target)
          
          
      #repetitions to be done
          # rf: explore the results of the following
                # n: {3, 4, 5, 6, 7}
          # repeated training-test split - do the 80/20 random sampling 9 more times?
                # repeat this 49 times, is there a more efficient way?
